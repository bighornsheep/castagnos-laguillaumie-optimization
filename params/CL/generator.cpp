/**
 ...
 @pre
    Creat a directory-subdirectory structure where directory name is the f-bit length and contains
    subdirectories which are DK-bit lengths.
...
int gen (const int N, const int t, const int fbits, const int dkbits)
{
    ...
    // Compute bit length of the product of primes of f and no. of primes in DK
    if (fbits <= fmaxbits)
    {
        factbits   = (int) ceil (((double) fbits) / t);
        dk_p_count = N + 1; // because DK needs q in this case
    }
    else
    {
        factbits   = dkbits;
    ...
    if ( !( access ( f_file, F_OK) != -1 ) ) // condition for non-existence
    {
        ...
        for ( int count = 1; count < PARAMETER_COUNT + 1; count++)
        {
            ...
            // When fbits < fbitsmax, check bit-length f and regenerate a prime of f if bit-length of f < fbits
            if (fbits <= fmaxbits)
            {
                // Check size first
            ...
            std::cout << mpz_sizeinbase(f , 2) << "\t";
            std::cout << mpz_sizeinbase(dk, 2) << "\t";
            std::cout << N                     << "\t";
            std::cout << t                     << "\t";
            std::cout << count                 << "\n";
        }
